import { findMatch } from '../utils/keywordUtils';
...
export const categoryKeywords = {
  ...
  beauty: [
    '화장', '메이크업', '스킨케어', '피부관리', '마사지', '네일', 
    '헤어', '미용실', '관리', '팩', '마스크팩', '뷰티케어'
  ],
  finance: [
    '재테크', '투자', '주식', '저축', '금융', '보험', '펀드',
    '자산관리', '가계부', '돈관리', '재무'
  ],
  pet: [
    '반려동물', '강아지', '고양이', '펫', '산책', '동물병원',
    '그루밍', '애완동물', '펫케어', '반려견', '반려묘'
  ],
  meditation: [
    '명상', '마음챙김', '힐링', '치유', '마인드풀니스', '요가',
    '정신수양', '이완', '심신안정', '마음수련'
  ],
  writing: [
    '글쓰기', '일기', '다이어리', '블로그', '작문', '독후감',
    '서평', '감상문', '기록', '저널링'
  ]
};
...
export const moodKeywords = {
  ...
  grateful: [
    '감사', '고마움', '감동', '은혜', '고맙다', '감사해',
    '감사한', '고마워', '감사함', '감동이야'
  ],
  proud: [
    '뿌듯', '자부심', '성취감', '보람', '자랑스러움', '뿌듯해',
    '해냈다', '성공', '달성', '이루었다'
  ],
  lonely: [
    '외로움', '쓸쓸함', '고독', '허전함', '그리움', '외롭다',
    '쓸쓸해', '혼자', '허전해', '그립다'
  ],
  anxious: [
    '불안', '걱정', '초조', '근심', '불안해', '걱정돼',
...
export function findCategory(text) {
  return findMatch(text, categoryKeywords, '카테고리');
}

export function findMood(text) {
  return findMatch(text, moodKeywords, '감정');
}

export function findWeather(text) {
  return findMatch(text, weatherKeywords, '날씨');
}